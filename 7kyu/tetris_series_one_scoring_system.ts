...
// scoring formula: mroe difficult line clears should be awarded more points
// ie single line clear = 40, 4 clear lines = 1200

// level multiplier: 0,...++ every ten lines cleared, and never reset
// refer to table

let basePoint: number[] = [40,100,300,1200]

export function getScore(arr: number[]) {
  
  let levelUp: number = 10;
  let level: number = 0;
  ...
  for(let val of arr) {
    ...
    if(val !== 0) {
      finalScore = finalScore + (basePoint[val-1] * (level + 1));
    }
    
    if (linesCounter >= levelUp) {
      level++;
      levelUp += 10;
    }
  }
